...
(define (count-leaves tree) 
 (tree-manip tree 0 (lambda (n) 1) +))

;2b.
(define (sum-leaves tree) 
  (tree-manip tree 0 (lambda (n) n) +))

;2c.
(define (triple-leaves tree) 
  (tree-manip tree '() (lambda (n) (* n 3)) cons))
...
;;  (foldl '<??> '<??> (map '<??> '<??>)))
  (foldl + 0 (map (lambda (n) (if (pair? n)
                             (count-leaves-with-map n) 1)) t)))
...
(define (buck elem lst)

  (cond ((null? lst) (list (list elem)))
      ((= elem (caar lst))
       (cons (cons elem (car lst)) (cdr lst)))
      (else (cons (list elem) lst))))
