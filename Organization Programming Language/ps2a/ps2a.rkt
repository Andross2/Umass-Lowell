...
(define (sum-iter sum counter max-count)
  (if (> counter max-count)
      sum
      (sum-iter (+ counter sum)
                (+ counter 1)
                max-count))
  )
;; what is the TIME complexity of this process?
...
(define (sum-squares-recursive n)
  (if (= n 0)
      0
      (+ (* n n) (sum-squares-recursive (- n 1))))
  )

;; write a recursive procedure for the same
;; that produces an ITERATIVE process
(define (sum-squares-iterative n)
   (sum-squares-iter 0 0 n)
  )

(define (sum-squares-iter sum counter max-count)
  (if (> counter max-count)
      sum
      (sum-squares-iter (+ (* counter counter) sum)
                (+ counter 1)
                max-count))
  )
...
;(define (sum-alt-squares-recursive n (x 1) (s 0)) ;; n is input, x = 1, s = 0 (sum) -- option input x and s. use for recursive
;  (if (= n 0);; check if n is 0
;      s      ;; return s if it true
;      (sum-alt-squares-recursive (- n 1) ;; n counter subtract by 1
;                                 (+ x 2) ;; x value start at 1 and increase by 2
;                                 (+ s (* x x)))) ;; add value x^2 to sum
(define (sum-alt-squares-recursive n)
  (if (= n 0)
      0
      (+ (* (- (* 2 n) 1) (- (* 2 n) 1)) (sum-alt-squares-recursive (- n 1))))
...
(define (sum-alt-squares-iterative n)
  (sum-alt-squares-iter n 1 0)
  )

(define (sum-alt-squares-iter n counter sum)
  (if (= n 0)
      sum
      (sum-alt-squares-iter (- n 1)
                            (+ counter 2)
                            (+ sum (* counter counter))))
  )
...
(define (two-thirds-series-r n)
  (cond 
         [(= n 0) 1]
         [(even? n) (+ (two-thirds-series-r (- n 1)) (expt 2 (- 0 n)))]
         [(odd? n) (- (two-thirds-series-r (- n 1)) (expt 2 (- 0 n)))]
   )
  )
...
(define (two-thirds-series-i n)
  (two-thirds-series-i-helper 0 0 n)
  )
(define (two-thirds-series-i-helper sum counter n)
  (if (> counter n)
      sum
      (cond
        [(= n 0) 1]
        [(odd? counter) (two-thirds-series-i-helper (- sum (expt 2 (- 0 counter))) (+ counter 1) n)]
        [(even? counter) (two-thirds-series-i-helper (+ sum (expt 2 (- 0 counter))) (+ counter 1) n)]
        )))
