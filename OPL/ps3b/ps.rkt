...
(define (sum-list-i lst)
  (define (sum-list-helper sum lst)
    (if (null? lst)
        sum
        (sum-list-helper (+ (car lst) sum) (cdr lst))))
  (sum-list-helper 0 lst))
...
(define (list-prod-iter lst)
  (define (list-helper lst list-prod)
    (if (null? lst)
        list-prod
        (list-helper (cdr lst) (* (car lst) list-prod))))
  (list-helper lst 1))
...
(define (same-parity first . lst)
  (define (sp-helper n element)
      (if (null? n) element
          (sp-helper (cdr n)
                     (if (= (remainder (car n) 2) (remainder first 2))
                         (append element (list (car n)))
                         element))))
  (sp-helper lst (list first)))
